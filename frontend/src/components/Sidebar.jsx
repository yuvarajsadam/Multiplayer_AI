...
import { Sparkles, History, Plus, Layers, Shield, Key, Hash, Check } from 'lucide-react';
...
const Sidebar = ({ onOpenRoomModal, onOpenVersionModal, onOpenAuthModal }) => {
  ...
  return (
    <aside className="w-80 bg-dark-800/90 border-r border-slate-800 flex flex-col h-full shrink-0 select-none z-10">
      {/* Sidebar Header */}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="space-y-1 max-h-36 overflow-y-auto pr-1">
              {joinedRooms.map((r) => {
                ...
                return (
                  <button
                    key={r.roomId}
                    onClick={() => joinRoom(r.roomId, r.name)}
        ...
        {/* AI Usage Metrics Progress Tracker */}
        {user ? (
          <UsageMetricsCard />
        ) : (
          <div className="bg-gradient-to-r from-indigo-900/40 to-violet-900/40 border border-indigo-500/30 rounded-xl p-3.5 space-y-2">
            <div className="text-xs font-bold text-indigo-300 flex items-center gap-1.5">
              ...
              <span>SaaS Account Integration</span>
            </div>
            <p className="text-[11px] text-slate-300 leading-tight">
              Sign in to track AI token usage, save custom workspaces, and manage multi-tenant access.
            </p>
            <button
